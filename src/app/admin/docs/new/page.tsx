...
export default function NewDocPage() {
  ...
  const [blocks, setBlocks] = useState<ContentBlock[]>([
    { id: "1", title: "Pendahuluan", content: "" },
  ]);
  ...
  useEffect(() => {
    const draft = sessionStorage.getItem("new_doc_draft");
    if (draft) {
      try {
        ...
        if (parsed.seriesSlug) setSeriesSlug(parsed.seriesSlug);
    ...
    return () => {
      ...
      if (isNavigatingAway.current) {
        sessionStorage.removeItem("new_doc_draft");
      }
    };
  }, []);
  ...
  useEffect(() => {
    if (isLoaded) {
      const draft = { title, description, seriesSlug, seriesOrder, blocks };
      sessionStorage.setItem("new_doc_draft", JSON.stringify(draft));
    }
  }, [title, description, seriesSlug, seriesOrder, blocks, isLoaded]);
  ...
  const handleSubmit = async (e: React.FormEvent, status: "DRAFT" | "PUBLISHED" = "DRAFT") => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showAlert({ type: "error", title: "Gagal Disimpan", message: error.message || "Failed to create document" });
      }
    } catch {
  ...
  return (
    <div className="w-full">
      <PageBanner title="Create New Document" backHref="/admin/docs" backText="Back">
        <button
          type="button"
